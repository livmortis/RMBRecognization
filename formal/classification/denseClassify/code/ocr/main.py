...
class DenseNet121(nn.Module):
    ...
    def __init__(self, out_size):
        ...
        #xzy 本地因为 'module name can\'t contain "."'的问题加载不了预训练参数，改为取消预训练。
...
def train_eval(epoch, model, train_loader, loss, optimizer, best_f1score=0, phase='train'):
    ...
    for i,data in enumerate(tqdm(train_loader)):
        ...
        if use_gpu:
            ...
            images, labels = [Variable(x) for x in data[1:3]]
            weights = torch.from_numpy(get_weight(labels))
...
def main():
    word_index_dict = json.load(open(args.word_index_json,encoding="utf-8")) #一个汉字对应一个编号
