...
def stati_image_size(image_dir, save_dir, big_w_dir):
    ...
    for image in os.listdir(image_dir):
        ...
        r = int(h / 8. / w)     # r=1 if h/w>8 ; r=0 if h/w<8
        h = h / 10
        w = w / 10
...
def main():
    ...
    save_dir = '../../files/'           #新建的
    big_w_dir = '../../data/big_w_dir'  #新建的
    stati_image_size(image_dir, save_dir, big_w_dir)

    train_label_json = '../../files/train_alphabet.json'  #已有的通过map_word_to_index脚本生成的标签
    long_text_dir = '../../data/long_text_dir'  #新建的
